...
## 3. Адресные и Групповые Заявки (NEW)

### 3.1. Логика Назначения
Оператор или Менеджер могут назначить заказ конкретному закупщику или группе закупщиков.

### 3.2. Отображение
- **Групповое назначение:** Если заказ назначен нескольким закупщикам (например, "Группа Шанхай"), он отображается в списке "НОВЫЕ" как обычная заявка, но с **бейджиком "Адресовано"** (иконка человечков + счетчик).
- **Эксклюзивное назначение:** Если заказ назначен **только одному** закупщику (вам), он приобретает особый статус:
    - **Визуально:** Карточка подсвечивается мягким **изумрудным свечением** (анимация `pulse + shadow`), привлекая внимание к срочности.
    - **Приоритет:** Такие заказы визуально выделяются из общего потока.
- **Изоляция:** Закупщик видит в списке только:
    1. Общие заказы (никому не назначенные).
    2. Заказы, где он есть в списке назначенных.
    *Чужие приватные заказы полностью скрыты.*

### 3.3. Жизненный цикл выделения
Анимированное свечение ("Эксклюзив") активно только до момента начала работы. Как только закупщик подает первый оффер (или черновик), статус заявки меняется, и визуальное выделение пропадает, переходя в обычный режим "В торгах".

---

## 4. Дашборд KPI (`BuyerDashboard.tsx`)
// ... (existing content)
