...
2. **Авто-расчет Срока:** 
   - **До выбора (Динамический):** Система берет срок закупщика (нед.) + значение из настроек (`delivery_weeks_add`).
   - **После выбора (Фиксированный):** При нажатии "Выбрать" (Лидер) текущий рассчитанный срок навсегда записывается в БД (`client_delivery_weeks`). Это защищает старые заказы от изменения глобальных настроек.
...
- **debug_mode:** Булево значение. Включает/выключает инструменты отладки (очистка БД, генерация данных, эмуляция ИИ) для всех ролей.
