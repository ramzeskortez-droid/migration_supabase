...
### 2.3. Целостность данных (Offer Items)
- **Уникальность:** Для таблицы `offer_items` установлен уникальный индекс `(offer_id, order_item_id)`. 
- **Логика:** Это гарантирует, что у одного закупщика может быть только одно предложение по конкретной позиции заказа. При повторном сохранении используется механизм `upsert` с разрешением конфликта (`onConflict`), что обновляет существующую запись вместо создания дубликата.
