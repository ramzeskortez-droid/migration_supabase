...
*   **Механизм (Dual Mode):**
    1.  **Локальный Worker (Primary):** Node.js скрипт (`sync_emails.cjs`) в режиме **IMAP IDLE**. Обеспечивает мгновенную (Push) доставку писем (< 1 сек).
    2.  **Supabase Edge Function (Backup):** Работает по Cron (раз в 1-5 мин) для подстраховки, если локальный скрипт выключен.
*   **Конфликты:** Если работают оба, Cron может "перехватить" письмо (пометить прочитанным) до того, как его увидит локальный скрипт.
*   **Фильтрация:** 
    *   **Тема:** Только письма, начинающиеся с "ЗАЯВКА" (регистронезависимо).
    *   **Дата:** Фильтр `SINCE` (например, от `Jan 06 2026`) для отсечения старой истории и проблем с часовыми поясами.
*   **Data Flow:**
    1.  Gmail получает письмо -> Push-уведомление (IDLE) скрипту.
    2.  Скрипт скачивает письмо, парсит (`mailparser`).
    3.  Проверка темы -> Если ОК, запись в `incoming_emails`.
    4.  Supabase Realtime мгновенно обновляет интерфейс Оператора.
