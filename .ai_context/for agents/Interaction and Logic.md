...
2.  **Авто-расчет Цены (Pricing Engine v2):**
    Система на лету (или при сохранении) считает итоговую цену.
    *   **Формула:** `Цена = ((ЦенаЗакупа_CNY * CNY_RUB) + (Вес * Тариф_$ * USD_CNY * CNY_RUB)) * (1 + Наценка/100)`.
    *   Все промежуточные и итоговые значения округляются до рубля.
    *   *Источник курсов:* Таблица `exchange_rates`. Поле `usd_rub` более не используется.
...
## 4. Коммуникация и Архивация

### 4.1. Логика Авто-Архивации
Чат считается активным только на этапе сбора предложений.
- **Условия архивации:**
    1.  С момента **последнего сообщения** в диалоге прошло 7 дней.
    2.  ИЛИ Статус Админа заказа не равен "В обработке".
    3.  ИЛИ Статус Поставщика заказа не равен "Сбор предложений" (начались торги).
- **Разархивация:** При отправке сообщения в архивный чат, вся история диалога восстанавливается.

### 4.2. Удаление чатов
Оператор и закупщик могут мгновенно удалить историю чата (иконка корзины). Окно подтверждения отключено.

---

## 5. Безопасность и Доступ (Security Model)
