...
- **File Management (Global):**
  - **Operator UI:**
    - Реализована полноценная мульти-загрузка файлов для позиций заказа (как у Закупщика).
    - В таблице товаров добавлена дропзона с поддержкой Drag&Drop и отображением количества файлов.
    - В блоке "Файлы по заявке" теперь отображаются ВСЕ файлы (общие + позиционные).
    - Файлы позиций помечены лейблом `Поз. X` и могут быть удалены прямо из общего списка.
    - Кнопки удаления файлов теперь появляются только при наведении (hover) для чистоты интерфейса.
  - **Manager UI:**
    - В таблице товаров (`AdminItemsTable`) добавлена поддержка загрузки файлов при редактировании позиции.
    - Реализована широкая дропзона для удобства.
    - Файлы, добавленные менеджером, сохраняются в `item_files`.
- **UI/UX:**
  - **Header:** Исправлено отображение курсов валют. Теперь выводятся только `¥/₽` и `$/¥` (из настроек), а курс `$/₽` рассчитывается автоматически как кросс-курс. Все числа форматируются с запятой (13,50).
  - **Fix:** Устранена проблема с перезаписью стейта при последовательной загрузке файлов в `PartsList` (атомарное обновление полей).

## 1.18.38 - Настройки Менеджера и коммуникации
- **Manager UI:**
  - Реализован метод `refuseOrder` в сервисном слое, исправлен баг аннулирования заказа.
  - Улучшена логика расчета сроков: теперь к значению закупщика автоматически прибавляется **+4 недели** (бизнес-правило).
  - В шапку заказа добавлено отображение файлов от поставщика с агрегацией данных из всех офферов и позиций.
  - Устранена мертвая зона при наведении на список файлов (ховер-эффект).
- **Communication:**
  - В чате внедрена цветовая дифференциация ролей: Менеджер (Зеленый), Оператор (Синий), Закупщик (Желтый).
  - Глобальное окно чата теперь корректно перекрывает хедер таблицы (z-index fix).
  - Добавлены кнопки быстрого чата с конкретным участником прямо из карточки заказа.
- **Security & DB:**
  - Создана миграция `72_add_refusal_reason.sql` для фиксации причин отмены заказов.
  - Удален ограничивающий constraint в `chat_messages` для поддержки новых ролей MANAGER и OPERATOR.

## 1.18.37 - Система одноразовых инвайт-кодов
- **Security:**
  - Внедрена система регистрации по одноразовым инвайт-кодам для всех ролей.
  - Убран мастер-пароль `china-nai` для операторов в пользу персональных инвайтов.
  - Реализована безопасная активация кода через RPC-функцию `use_invite_code` (атомарная проверка и пометка использования).
- **Manager UI:**
  - Добавлен раздел "Инвайты" в управлении пользователями.
  - Менеджер может генерировать уникальные коды (6 знаков) отдельно для закупщиков и операторов.
  - Реализован список активных (неиспользованных) инвайтов.
- **Auth UI:**
  - Обновлены формы регистрации Оператора и Закупщика: добавлено поле ввода инвайт-кода.
  - Реализована обязательная модерация: после регистрации аккаунт попадает в статус `pending` и требует одобрения менеджером.
- **Database:** Созданы таблицы `invite_codes` и политики доступа для анонимных сессий.

## 1.18.36 - Исправление аннулирования и интеграция чата
- **Manager UI:**
  - Восстановлен и исправлен функционал аннулирования заказа (добавлен метод `refuseOrder`).
  - Увеличен `z-index` модального окна отмены до 9999 для предотвращения перекрытия хедером.
  - Исправлена ошибка "onOpenChat is not a function" за счет корректного проброса пропсов.
  - В `AdminOrdersList` добавлена иконка `MessageCircle`.
- **Chat Integration:**
  - `GlobalChatWindow` теперь поддерживает автоматическое открытие конкретного диалога при переходе из заказа.
- **Database:**
  - Создана миграция `72_add_refusal_reason.sql` для хранения причин отмены.
  - Создана миграция `71_fix_chat_constraint.sql` для поддержки новых ролей в чате.

## 1.18.35 - Чат Менеджера и цветовое кодирование
- **Manager UI:**
  - Добавлена кнопка "Глобальный чат" в шапку с бейджем уведомлений о новых сообщениях.
  - Реализован быстрый переход в чат с конкретным закупщиком из таблицы офферов.
  - Добавлена кнопка "Чат с оператором" в футер развернутого заказа.
  - Исправлен баг аннулирования заказа (кнопка подтверждения теперь работает корректно).
- **Chat Engine:**
  - Внедрено цветовое кодирование ролей: Менеджер (Зеленый), Оператор (Синий), Закупщик (Желтый).
  - Обновлена логика ролей: теперь сообщения сохраняются с ролями `MANAGER` и `OPERATOR` вместо общего `ADMIN`.
- **Optimization:** Удалены отладочные логи из консоли.

## 1.18.34 - Улучшение управления файлами и финализация редизайна
- **Manager UI:**
  - Блок файлов разделен на "Файлы от оператора" и "Файлы от поставщиков".
  - Реализована полная агрегация файлов поставщика: в шапке заказа отображаются все общие документы и файлы из каждой позиции всех офферов.
  - Исправлен выпадающий список файлов в таблице офферов: устранена "мертвая зона" при наведении, файлы теперь легко открываются.
  - Удален избыточный горизонтальный скролл в таблице предложений.
- **Buyer UI:**
  - Блок вложений переработан в компактный строчный вид.
  - Файлы позиций теперь автоматически попадают в общий список вложений с указанием номера позиции (#).
  - Реализовано удаление любых файлов закупщика прямо из списка вложений (с привязкой к источнику).
- **Shared:**
  - Dropzone в таблице теперь занимает всё доступное пространство ячейки.
  - Оптимизирована логика превью: миниатюра отображается только для одного изображения, для нескольких файлов выводится иконка с количеством.

## 1.18.33 - Редизайн интерфейса Закупщика и мульти-файлы
- **Buyer UI Redesign:**
  - Полная переверстка развернутого заказа по новому референсу.
  - Реализован табличный вид позиций с прилипающим (sticky) футером действий.
  - Инпуты закупщика (цена, вес, срок) вынесены в отдельную темную плашку внутри каждой позиции.
  - Поля "Комментарий" и "Поставщик" объединены в двухколоночную строку с заголовками.
- **File Management:**
  - Реализована мульти-загрузка файлов для каждой позиции и для заказа в целом.
  - Управление вложениями (просмотр и удаление) перенесено в общий блок под списком товаров.
  - Файлы позиций помечаются соответствующим номером (напр. #1).
- **UX Improvements:**
  - Новый визуальный эффект отказа от позиции: красное зачеркивание всей строки и блокирующий оверлей с кнопкой возврата.
  - Исправлено "мерцание" дропзоны при перетаскивании файлов.
  - Увеличен приоритет (z-index) футера для корректного перекрытия контента.

## 1.18.32 - Настройки обязательности и поле Поставщика
- **Database:** 
  - Добавлена колонка `supplier_sku` в таблицу `offer_items`.
  - Создана таблица `system_settings` для хранения глобальных настроек платформы.
- **Manager UI:**
  - Реализован новый раздел **"Настройки"** в панели управления.
  - Добавлена возможность настраивать обязательность заполнения поля "Поставщик" для закупщиков.
- **Buyer UI:**
  - В карточку товара добавлено поле **"Поставщик (Название/№)"**.
  - Реализована динамическая валидация: если поле отмечено в настройках как обязательное, оно помечается звездочкой `*`, а кнопка отправки предложения блокируется до заполнения поля.
- **Service:** Удалены отладочные логи из консоли.
