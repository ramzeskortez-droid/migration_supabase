...
- **Chat Logic:**
  - Обновлен метод `getGlobalChatThreads`: теперь он принимает роль текущего пользователя.
  - Для **Закупщика**: реализована группировка сообщений по собеседнику ("Менеджер" vs "Оператор"). Теперь закупщик видит две отдельные ветки в рамках одного заказа, если общается с разными ролями.
  - Для **Оператора/Менеджера**: сохранена группировка по Имени Поставщика (каждый поставщик - отдельная ветка).
