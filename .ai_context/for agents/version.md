...
- **Maintenance:**
  - Создана папка `_legacy` для хранения неактуальных файлов (старые SQL миграции, CSV дампы, локальные BAT скрипты).
  - Удалена временная папка `temp/`.
  - В архив `_legacy` перемещен файл `vercel.json`, так как деплой переносится на SpaceWeb.
- **Git Configuration:**
  - Обновлен `.gitignore`: добавлены правила для игнорирования папки `_legacy`, дампов базы (`*.csv`, `*.sql`) и локальных инструментов разработки (`*.bat`, кроме основного прокси-файла).
- **Environment:**
  - Корень проекта очищен от мусора для удобства работы и чистого деплоя фронтенда.

## 1.18.110 - Переход на Mail.ru и улучшение обработки почты
- **Edge Function (`fetch-emails`):**
  - Сбор заявок переключен с Gmail на `china.flow@mail.ru`.
  - Внедрена библиотека `mailparser` для автоматической очистки тела письма от MIME-заголовков и кодировок (base64). Теперь оператор видит чистый текст.
  - Добавлен фильтр по дате: обрабатываются только письма, начиная с 14.01.2026.
  - Убран фильтр по теме "ЗАЯВКА": теперь в систему попадают все входящие письма.
  - Реализована дедупликация: перед вставкой проверяется наличие письма с такой же темой и отправителем за последние 2 дня.
  - Расширено логирование процесса обработки в консоль Supabase.

## 1.18.109 - Fix: Manager Privacy & Thread Names
- **Privacy:** Исправлена утечка сообщений Закупщика в чат "Менеджер-Оператор". Теперь Менеджер видит только переписку с Оператором.
- **UI:** Исправлено название треда у Оператора при входящих сообщениях от Менеджера (теперь отображается как "Менеджер", а не системное имя).
- **Logic:** В `GlobalChatWindow` добавлено автоматическое определение `threadRole` для корректной фильтрации сообщений.
