## 1.18.145 - UI: Collapsible Client Info Section in Admin Panel
- **Админ-панель:**
  - Раздел "Информация о клиенте" в развернутом виде заказа теперь по умолчанию свернут, чтобы уменьшить вертикальное пространство.
  - Добавлена возможность разворачивать/сворачивать этот раздел по клику на заголовок.
  - Оптимизированы отступы, чтобы свернутый раздел был более компактным.
  - Поле "Адрес" теперь занимает всю ширину контейнера для корректного отображения длинных адресов без переносов.

БЫЛА ПРОБЛЕМА:
Раздел "Информация о клиенте" занимал много места, даже когда был не нужен. Длинные адреса не помещались в выделенное пространство и переносились.

КАК РЕШИЛИ:
1. В `AdminOrdersList.tsx` добавлен `useState` для управления состоянием видимости раздела.
2. Реализован условный рендеринг содержимого блока и добавлены иконки-переключатели.
3. Скорректированы классы отступов (`padding`) для компактного вида в свернутом состоянии.
4. Изменена сеточная структура (`grid`) для полей, чтобы "Адрес" занимал всю ширину.

# Version History

## 1.18.144 - Fix: Sorting Logic
- **Админ-панель:**
  - Исправлена сортировка заказов (ранее сортировка всегда сбрасывалась на ID из-за ошибки в передаче параметров). Теперь корректно работает сортировка по Дедлайну, Дате и др.
- **API (Ядро):**
  - Добавлена поддержка сортировки по ключу `status_manager` (исправляет сортировку по статусу у Оператора).

БЫЛА ПРОБЛЕМА:
1. В Админке сортировка по "Срок до" работала хаотично (фактически сортировало по ID).
2. У Оператора не работала сортировка по Статусу.

КАК РЕШИЛИ:
1. Пробросили параметр `sortBy` из стейта `AdminInterface` в хук `useOrdersInfinite`.
2. Добавили `status_manager` в маппинг колонок в `fetch.ts`.

## 1.18.143 - Admin: Исправление редактирования даты при повторе заказа
- **Админ-панель:**
  - Исправлена ошибка валидации поля "Срок до" при повторении заказа (Repeat Order).
  - Реализована конвертация формата даты (DD.MM.YYYY -> YYYY-MM-DD) при инициализации формы редактирования.
  - Теперь исходный дедлайн корректно подставляется в календарь.

БЫЛА ПРОБЛЕМА:
При нажатии "Повторить заказ" открывалась форма редактирования с пустым полем даты (из-за неверного формата), что блокировало сохранение.

КАК РЕШИЛИ:
Добавили функцию `toIsoDate` в `AdminInterface.tsx` для преобразования строки даты перед записью в стейт формы.
