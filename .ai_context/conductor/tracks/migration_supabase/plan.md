...
## Фаза 6: Оптимизация и Рефакторинг (✅ Готово)
- [x] **Hotfix:** Исправление сохранения стоимости доставки (`delivery_rate` vs `delivery_days`).
- [x] **Performance:** Внедрение RPC-функции `approve_order_winners` для мгновенного утверждения КП (Atomic Transaction).
- [x] **Server-Side Logic:** Перенос расчетов стоимости (`total_cost`, `goods_cost`) на сторону БД (Computed Columns).
- [x] **UX/UI:** 
    - [x] Добавление колонки времени (`status_updated_at`).
    - [x] Умная сортировка (Новые -> Офферы, Остальные -> Время).
    - [x] Исправление "исчезающих" заказов при смене статуса.
- [x] **Refactoring:** Полная декомпозиция `ClientInterface.tsx` на подкомпоненты (`hooks`, `shared`, `client/`).
