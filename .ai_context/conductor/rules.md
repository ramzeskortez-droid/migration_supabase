...
8. **Отчетность и GIT (КРИТИЧНО):**
   - **GIT PUSH:** **КАТЕГОРИЧЕСКИ ЗАПРЕЩЕНО** делать `git push` без прямой команды пользователя.
   - **GIT COMMIT:** Разрешено делать самостоятельно после выполнения задачи, но **строго** по шаблону ниже.
