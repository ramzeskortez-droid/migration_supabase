...
8. **Отчетность и GIT (КРИТИЧНО):**
   - **ВНИМАНИЕ: КАТЕГОРИЧЕСКИ ЗАПРЕЩЕНО** делать `git commit` или `git push` без прямой команды пользователя ("Сделай комит", "Отправь" и т.д.).
   - Даже если задача кажется завершенной, жди команды. 
   - Нарушение этого правила считается критическим сбоем агента.
   ...
   - По завершении: краткий список "Что сделано" и блок "Как проверить".
     ...
     - **Обязательная структура коммита (СТРОГО):**
       ```text
       ВЕРСИЯ [X.X.X] - [Название]
       - [Изменение 1]
       - [Изменение 2]
   
       БЫЛА ПРОБЛЕМА:
       [Четкое описание бага или отсутствующей функции]
     ...
     - *Пример:*
       ```text
       ВЕРСИЯ 1.18.121 - Управление Официальными Брендами
       - Добавлены табы "Все бренды" и "Официалы" в админ-панели.
       - Реализована возможность помечать бренды как "Официальные".
   
       БЫЛА ПРОБЛЕМА:
       Отсутствовала возможность выделять официальных представителей брендов.
   
       КАК РЕШИЛИ:
       1. Расширили API брендов (flag official).
       2. Добавили визуальные акценты в UI.
       ```
     - **ЗАПРЕЩЕНО:** Использовать `git commit --amend` в цепочке команд.
