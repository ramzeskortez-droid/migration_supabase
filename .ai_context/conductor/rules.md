...
7. **Работа с багами (bugs_log.md):**
   ...
   - **ТРЕБОВАНИЕ К ДЕТАЛИЗАЦИИ:** 
     - **Проблема:** Описывай максимально подробно. Включай прямые цитаты пользователя, симптомы (что видел пользователь), условия возникновения и техническую суть (ошибки в консоли, сетевые запросы).
     - **Решение:** Описывай пошагово. Указывай конкретные файлы, измененные функции, изменения в логике и почему выбран именно этот путь решения.
   - **ФОРМАТ ЗАПИСИ:**
     ```markdown
     ## [Дата] - [Краткое название проблемы]
     **Версия:** [Текущая версия]
     **Проблема:** [Глубокое описание: слова пользователя, симптомы, влияние на систему]
     **Причина:** [Технический корень проблемы: баг в коде, пропущенный пропс, ограничение БД и т.д.]
     **Решение:** [Детальный разбор: пошаговые изменения в коде, затронутые модули, примененные фиксы]
