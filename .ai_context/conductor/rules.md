...
8. **Отчетность и GIT (КРИТИЧНО):**
   ...
   - **Заголовок:** `ВЕРСИЯ [X.X.X] - [Краткое название]`
   ...
   - **ЗАПРЕЩЕНО:** Использовать `git commit --amend` в цепочке команд (через `&&`), так как парсер безопасности блокирует это действие. Используй отдельные команды или избегай amend.
