...
export const editOffer = async (offerId: string, items: any[], supplierFiles?: any[]): Promise<void> => {
    ...
    const upsertData = items.map(item => {
        ...
        return {
            ...
            name: item.name,
            quantity: item.offeredQuantity !== undefined ? item.offeredQuantity : item.quantity,
            price: item.BuyerPrice || item.sellerPrice || 0,
            currency: 'CNY',
            delivery_days: item.deliveryWeeks ? item.deliveryWeeks * 7 : 0,
