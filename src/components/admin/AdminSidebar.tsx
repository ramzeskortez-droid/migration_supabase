...
import { LayoutDashboard, ClipboardList, Settings, Banknote, Tag, Users, Sliders, CheckSquare } from 'lucide-react';

interface AdminSidebarProps {

  currentView: 'listing' | 'statuses' | 'finance' | 'brands' | 'users' | 'settings' | 'checklist';

  setCurrentView: (view: 'listing' | 'statuses' | 'finance' | 'brands' | 'users' | 'settings' | 'checklist') => void;
...
export const AdminSidebar: React.FC<AdminSidebarProps> = ({ currentView, setCurrentView, debugMode }) => {

  return (



    <aside className="group w-20 hover:w-64 bg-white border-r border-slate-200 p-4 flex flex-col gap-2 shrink-0 transition-all duration-300 ease-in-out overflow-hidden z-30 h-screen sticky top-0">
      ...
      >
        ...
        <span className="opacity-0 group-hover:opacity-100 transition-opacity duration-300">Финансы</span>
                  ...
                  <button 
                    ...
                    className={`flex items-center gap-3 px-3 py-3 rounded-xl text-xs font-black uppercase transition-all whitespace-nowrap ${currentView === 'settings' ? 'bg-amber-600 text-white shadow-lg' : 'text-slate-400 hover:bg-slate-50'}`}
                  ...
                  >
                    ...
                    <div className="shrink-0"><Sliders size={18} /></div>

      

                    <span className="opacity-0 group-hover:opacity-100 transition-opacity duration-300">Настройки</span>
                  ...
                  {debugMode && (
                    ...
                    <>
                      ...
                      <button 
                        ...
                        className={`flex items-center gap-3 px-3 py-3 rounded-xl text-xs font-black uppercase transition-all whitespace-nowrap ${currentView === 'checklist' ? 'bg-indigo-600 text-white shadow-lg' : 'text-slate-400 hover:bg-slate-50'}`}
                      ...
                      >
                        ...
                        <div className="shrink-0"><CheckSquare size={18} /></div>

      

                        <span className="opacity-0 group-hover:opacity-100 transition-opacity duration-300">Чек-лист проверки</span>
