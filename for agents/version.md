...
## 1.9.39
- **Admin Interface:**
  - **Layout:** Список офферов переведен на Grid с относительными размерами (`fr`) для равномерного распределения колонок на любых экранах.
  - **Колонки:** Изменен порядок (Фото -> Доставка). Скрыт заголовок кнопки выбора.
  - **Ограничения:** После утверждения КП редактирование офферов блокируется (инпуты disabled, кнопка выбора скрыта).
- **Backend (GAS):**
  - **Critical Fix:** Исправлены индексы колонок в методах `form_cp` (Утверждение), `confirm_purchase` (Покупка) и `refuse_order` (Отказ). Ранее запись шла не в те колонки (сдвиг на 1), из-за чего статусы не обновлялись корректно.
  - **Workflow:** Добавлена поддержка колонки `WorkflowStatus` (Col O).
- **Файлы:** `components/AdminInterface.tsx`, `перенос/TG_BLOCK_V2.js`.

## 1.9.38
- **Admin Interface:**
  - **Performance:** Оптимистичное обновление интерфейса при утверждении КП (`executeApproval`) перенесено до сетевого запроса. Реакция на нажатие кнопки стала мгновенной (ранее была задержка 2-3 сек).
  - **Визуализация:** В список офферов добавлены новые поля: Вес (кг), Срок (нед.) и кнопка "Фото".
  - **Типографика:** Увеличен размер шрифта названия детали в заголовке оффера.
- **Client Interface:**
  - **Редизайн:** Полностью переработан блок "Согласованные позиции".
  - **Типографика:** Увеличен размер шрифта названия детали и цены.
  - **Данные:** Добавлены бейджи с Весом (кг) и Сроком доставки (недели).
  - **Фото:** Ссылка на фото теперь оформлена как полноценная кнопка "📷 ФОТО ДЕТАЛИ" вместо маленькой иконки.
- **Файлы:** `components/AdminInterface.tsx`, `components/ClientInterface.tsx`.

## 1.9.37
- **Seller Interface:**
  - **Новые поля:** В форму подачи оффера добавлены обязательные поля "Вес (кг)" и "Срок доставки (недели)", а также опциональное поле "URL фото".
  - **Валюта:** Подача цен теперь строго в CNY (Юань).
  - **Аналитика:** Добавлен индикатор "Лучшая цена сейчас" для каждой детали (сравнение с другими офферами к этому заказу).
  - **Feedback:** Реализовано отображение комментариев администратора к позициям оффера.
- **Admin Interface:**
  - **Обратная связь:** Добавлено текстовое поле ( Feedback / Admin Comment ) для каждой позиции оффера. Сохраняется при выборе поставщика.
  - **Отображение:** В список офферов выведены колонки Веса, Срока доставки и иконка ссылки на фото.
- **Client Interface:**
  - **Визуализация:** Добавлено отображение миниатюр фото и комментариев админа в блоке согласованных позиций.
- **Backend (GAS):** 
  - Обновлена логика `handleRankUpdate` для сохранения `adminComment` в JSON.
  - Улучшены Telegram-уведомления (вывод веса и срока в списке позиций).
- **Файлы:** `перенос/TG_BLOCK_V2.js`, `services/sheetService.ts`, `components/SellerInterface.tsx`, `components/AdminInterface.tsx`, `components/ClientInterface.tsx`, `types.ts`.

## 1.9.36
- **Backend (GAS):** 
  - Реализована **серверная генерация ID для офферов**. ID предложений теперь имеют формат `[OrderId]-[OfferNumber]` (например, `1-1`, `1-2`).
- **Service:**
  - `sheetService.ts` обновлен для поддержки серверной генерации ID офферов.
- **Docs:**
  - В `backlog.md` добавлены задачи: звуковые уведомления, экспорт в PDF/Excel, фильтр по датам.
- **Fix:**
  - Добавлен `vercel.json` для корректной работы SPA-маршрутизации на хостинге (исправление ошибки 404).
- **Файлы:** `перенос/TG_BLOCK_V2.js`, `services/sheetService.ts`, `conductor/backlog.md`, `vercel.json`.

## 1.9.35
- **Backend (GAS):** 
  - Реализована **серверная генерация ID** для заказов. Теперь ID назначаются строго последовательно (1, 2, 3...) на стороне сервера.
  - Добавлена функция `getNextId` с использованием блокировки `LockService` для предотвращения дублирования номеров.
  - Сервер теперь возвращает назначенный `orderId` в ответе на запрос создания заказа.
- **Client Interface:**
  - **Защита от дублей:** Внедрена блокировка кнопки "Отправить" (`isSubmitting`) до момента завершения сетевого запроса.
  - **UI/UX:** При создании заказа в колонке ID отображается лоадер, пока сервер не присвоит номер.
  - **Bugfix:** Исправлена критическая ошибка (краш страницы) при поиске/рендеринге заказов с числовыми ID. Внедрено принудительное приведение ID к строке.
- **Service:** 
  - Метод `SheetService.createOrder` переведен на ожидание ID от сервера.
  - Добавлена глобальная защита типов: все ID принудительно конвертируются в `string` при получении данных.
- **Файлы:** `перенос/TG_BLOCK_V2.js`, `services/sheetService.ts`, `components/ClientInterface.tsx`.
